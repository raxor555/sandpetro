...
export const Navbar: React.FC = () => {
  ...
  return (
    <header
      className={`fixed w-full z-40 transition-all duration-300 ${isScrolled || location.pathname !== '/' ? 'bg-white shadow-md' : 'bg-transparent'
        }`}
    >
      {/* Top Bar - Contact Info */}
      <div className={`py-2 px-6 hidden md:flex justify-between items-center text-xs font-medium tracking-wide transition-all duration-300 z-50 ${isScrolled || location.pathname !== '/' ? 'bg-petro-dark text-white' : 'bg-petro-dark/30 text-white backdrop-blur-sm'
        }`}>
        <div className="flex space-x-6">
          ...
          <a href="https://wa.me/966550143746" target="_blank" rel="noopener noreferrer" className="flex items-center hover:text-sand transition-colors">
      ...
      {/* Main Navbar */}
      <div
        className={`container mx-auto px-6 md:px-12 flex justify-between items-center transition-all duration-300 ${isScrolled || location.pathname !== '/' ? 'py-3' : 'py-5'
          }`}
      >
        {/* Logo */}
        ...
        {/* Mobile Menu Button */}
        <button
          className={`md:hidden ${isScrolled || location.pathname !== '/' ? 'text-petro' : 'text-white drop-shadow-md'}`}
